...
export function InscriptionForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
      ...
      const inscriptionData = {
        ...
        dateEcolage: paiementData.dateEcolage ? new Date(paiementData.dateEcolage).toISOString().split('T')[0] : '',
      ...
      try {
          console.log("Données envoyées à l'API:", inscriptionData);
          ...
          if (!parcoursType) {
              throw new Error("Le type de formation est requis");
          }
          ...
          if (!res.ok) {
              ...
              let errorMessage = `Erreur ${res.status}: ${statusText}`;
              ...
              try {
                  ...
                  if (contentType && contentType.includes('application/json')) {
                      ...
                      console.error("Erreur détaillée (JSON):", errorData);
                      
                      // Gestion du cas où errorData est un objet vide
                      if (Object.keys(errorData).length === 0) {
                          errorMessage = `Erreur ${res.status}: La requête est invalide. Veuillez vérifier les données envoyées.`;
                          console.error("L'API a renvoyé un objet vide. Vérifiez les données de la requête:", {
                              url: res.url,
                              status: res.status,
                      ...
                      // Si la réponse n'est pas du JSON, lire comme texte
                      const text = await res.text();
                      console.error("Réponse d'erreur (texte):", text);
